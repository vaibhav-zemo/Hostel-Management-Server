...
router.post(
  ...
  (req, res) => {
    ...
    room
      .save()
      .then((data) =>
        res.json({ success: true, message: "Room has been created." })
      )
      .catch((err) =>
        res.status(400).json({ ...err, message: "Error while creating room." })
      );
  }
);
...
router.delete(
  ...
  (req, res) => {
    ...
    Room.findByIdAndDelete(_id)
      .then((status) =>
        res.json({ succes: true, message: "Student has been deleted." })
      )
